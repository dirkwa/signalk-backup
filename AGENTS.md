...
- [src/config/image-tag.ts](src/config/image-tag.ts) — `imageTag` defaults to `"auto"`, which resolves to the newest published release as a concrete version via [src/config/resolve-auto-tag.ts](src/config/resolve-auto-tag.ts), with `BACKUP_SERVER_VERSION` as the offline floor. `"latest"` is available as an opt-out. Plugin and server versions stay decoupled.
