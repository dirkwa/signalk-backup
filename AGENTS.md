...
- [src/config/image-tag.ts](src/config/image-tag.ts) — `imageTag` defaults to `"latest"`. The opt-in `"auto"` resolves to the newest published release as a concrete version via [src/config/resolve-auto-tag.ts](src/config/resolve-auto-tag.ts), with `BACKUP_SERVER_VERSION` as the offline floor. Plugin and server versions stay decoupled.
...
- **The default `imageTag` is `"latest"`, and that choice has a known cost.** A floating tag makes signalk-container's `classifyTag` skip version comparison and use digest-drift detection instead, which calls `pullImage()` on *every* update check (~400MB per check on a metered link) and reports "image rebuild available" instead of `0.6.10 → 1.0.0`. This was chosen deliberately for a simpler mental model; don't "fix" it back without asking.
- **`imageTag: "auto"` is the opt-in alternative and must always resolve to concrete semver.** [src/config/resolve-auto-tag.ts](src/config/resolve-auto-tag.ts) picks, in order: the persisted `resolvedImageTag` (no network — this is why repeat boots reuse one image and never recreate the container), the newest published GitHub release of signalk-backup-server, then the `BACKUP_SERVER_VERSION` floor. The lookup is bounded (5s) and never throws, because boats boot without an uplink. It must never emit a floating tag, or it collapses into the `"latest"` behavior above. `BACKUP_SERVER_VERSION` is still worth bumping when a server release lands — it is the floor that offline and first-boot `"auto"` installs get.
